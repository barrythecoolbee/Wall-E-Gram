...
<?php
  ...
  if ( !isset($_SESSION) ) {
	...
	$query_posts = "SELECT * FROM `$db`.`rss-posts` where `idPost` = ".$idPost;
	$result_posts = mysqli_query($GLOBALS['ligacao'], $query_posts);

?>
<head>
    <link rel="stylesheet" type="text/css" href="../../Styles/style_home.css">
    <title>Wall-e-gram post</title>
	<script type="text/javascript" src="https://platform-api.sharethis.com/js/sharethis.js#property=60d2ffb39253ec0012a7d303&product=inline-share-buttons" async="async"></script>
</head>
<body>

<?php
	$posts_info = mysqli_fetch_array($result_posts);
...
?>
	...
	<div id="instacard" class="instacard">
		<div id="header" class="instacard-header">
			...
			<span style="margin-right: 16px; float: right; width: 100%;">
		...
		<div id="footer" class="footer">
			...
			<div class="comments">
				<div style="margin: 15px;">
					...
					<?php 
						if($posts_info['idUser'] == $_SESSION['id']){
					?>
						<div style="float: right; margin-right: 38px;" class="slider-to-right">
							<label class="switch-text">Private</label>
							<label class="switch">
								
							<?php
								if($posts_info['visibility'] == 0){
							?>
								<input onclick="location.href = 'processFormChangeVisibility.php?idPost=<?php echo $idPost ?>&visibility=1';" type="checkbox" id="switchValue" name="visibility">
							<?php
								} else{
							?>
								<input onclick="location.href = 'processFormChangeVisibility.php?idPost=<?php echo $idPost ?>&visibility=0';" type="checkbox" id="switchValue" name="visibility" checked>
							<?php
								}
							?>
								<span class="slider round" name="switch2"></span>
...
<?php
		...
		echo "<h1>This post is private</h1>";
	}
