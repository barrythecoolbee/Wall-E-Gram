...
function getParamFromField($param, $field, $value, $authType = "challenge") {
    ...
    $query = "SELECT `$param` AS param FROM `$dataBaseName`.`auth-$authType` " .
            "WHERE `$field`='$value'";
...
function addUserToDatabase($param1, $param2, $param3, $authType = "basic"){
    ...
    mysqli_select_db($GLOBALS['ligacao'], $dataBaseName );
    $newId = (int)getLastId("idUser", $authType);
    $query = null;
    if($authType == "basic"){
        //param1 = username; param2 = password; param3 = email
        $newId = $newId + 1;
        
        $query = "INSERT INTO `$dataBaseName`.`auth-$authType` " .
                "VALUES( $newId, '$param1', '$param2', '$param3', 0, 'NULL', 'NULL')";

    }
    else if($authType == "challenge"){
        $newId = (int)getLastId("idUser", "basic");
        //param1 = challenge; param2 = registerDate; param3 = null
         $query = "INSERT INTO `$dataBaseName`.`auth-$authType` " .
                "VALUES( '$newId', '$param1', '$param2')";
    }
    else if($authType == "permissions"){
        //param1 = idRoles; param2 = idUser;
        $query = "INSERT INTO `$dataBaseName`.`auth-$authType` " .
...
function updateField($field, $valueField, $param, $valueParam, $authType = "basic"){
    ...
    if($authType == "basic"){
        //field = active; param = idUser
        $query = "UPDATE `$dataBaseName`.`auth-$authType` " .
                "SET `$field` = $valueField WHERE `$param` = $valueParam";
    }
...
function removeLine($field, $value, $authType = "basic"){
    ...
    $query = "DELETE FROM `$dataBaseName`.`auth-$authType` " .
            "WHERE `$field` = '$value'";
...
function getLastId($field, $authType = "basic"){
  
    $dataBaseName = $GLOBALS['configDataBase']->db;

    mysqli_select_db($GLOBALS['ligacao'], $dataBaseName );

    if($authType == "post-images"){
        $query = "SELECT MAX(`$field`) AS id  FROM `$dataBaseName`.`$authType` " ;
        $result = mysqli_query($GLOBALS['ligacao'], $query ) ;
        $row = mysqli_fetch_array($result);

    } else if($authType == "rss-posts"){
        $query = "SELECT MAX(`$field`) AS id  FROM `$dataBaseName`.`$authType` " ;
        $result = mysqli_query($GLOBALS['ligacao'], $query ) ;
        $row = mysqli_fetch_array($result);
    }
    else{
        $query = "SELECT MAX(`$field`) AS id  FROM `$dataBaseName`.`auth-$authType` " ;
        $result = mysqli_query($GLOBALS['ligacao'], $query ) ;
        $row = mysqli_fetch_array($result);
    }
...
function getRole($userId) {
    ...
    while ($userData = mysqli_fetch_array($result)) {
        ...
        $userRole .= $userData['friendlyName'];
    }
    $userRoles .= "]";
...
function getFileDetails($ids) {
    ...
    $query = "SELECT * FROM `$dataBaseName`.`post-images` WHERE " . $whereClause;
